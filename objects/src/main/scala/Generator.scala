...
// trait SwaggerModelGenerator {
//   import SwaggerModelGenerator._

//   def swaggerSources: Sources =
...
//         .flatMap(swagger =>
//           processSwaggerFile(swagger.path, T.ctx.dest, T.ctx.log)
//         )
//         .map(PathRef(_))
//   }
// }
...
//     )
//     allowedPrefixes.exists(className.startsWith) && !skipClasses.contains(
//       className
//     )
//   }

//   def processSwaggerFile(swaggerFile: Path, outputDir: Path): Seq[Path] = {
...
//       sanitizeClassPath(split.init.mkString("/"))
//     ) / s"$className.scala"

//     val generatedClass =
//       definitionJson.as[Definition].fold(throw _, identity) match {
...
//             properties.toSeq.flatten.sortBy(_._1),
//             required.toSeq.flatten
//           )
...
//       properties: Iterable[(String, Property)],
//       required: Seq[String]
//   ): String =
//     properties.toSeq
//       .sortBy { case (name, _) =>
...
//         swaggerToScalaType(
//           t,
//           property.items.orElse(property.additionalProperties),
//           property.format
//         )
...
//           case f =>
//             sys.error(s"Unsupported format '$f' for swaggerType '$swaggerType'")
//         }
//       case ("object", Some(subProperty)) =>
//         s"Map[String, ${generateType(subProperty)}]"
...
//         }
//       case (swaggerType, _) =>
//         swaggerType.take(1).toUpperCase + swaggerType.drop(1)
//     }
...
//         s"$existingClassesPrefix.IntOrString"
